...
int main() {
    ...
    double ex_dot_x = dot(v_x, v_ex);
    double ex_dot_y = dot(v_y, v_ex);
    double ex_dot_z = dot(v_z, v_ex);

    std::cout << "Result of dot products: " << ex_dot_x << std::endl << ex_dot_y << std::endl << ex_dot_z << std::endl;
    ...
    vec3 v_ex_reconstruct = v_x * ex_dot_x + v_y * ex_dot_y + v_z * ex_dot_z;
