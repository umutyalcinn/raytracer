...
*/
    ...
    std::optional<Intersection> Scene::closest_intersection(const Ray& r,
        ...
        double local_min_t = std::numeric_limits<double>::max();
...
*/
        auto direction = light_source.light_direction_to(position);

        auto distance = light_source.distance_to(position);

        return Scene::any_intersection(cg::Ray(position, -direction), 0.001, distance);
    }
